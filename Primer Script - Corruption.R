...

{
  ...
  # Mapa opcional para binarias 1/2: cuál es el valor "positivo" (si está en 0/1 no afecta).
  affirmative_map <- c(
    voto = 1, partprot = 1, juntas_alto = 1,
    sobpol = 1, sobpub = 1, genero = 1, job = 1, govhelp = 1
  )
  
  bin_stats <- function(x, varname){
    xnum <- to_num(x)
    u <- sort(unique(xnum[!is.na(xnum)]))
    N <- sum(!is.na(xnum))                        
    rng <- if (length(u)) paste0(min(u), "–", max(u)) else "—"
    if (all(u %in% c(0,1))) {
      pct <- mean(xnum == 1, na.rm = TRUE) * 100
    } else if (all(u %in% c(1,2))) {
      pos <- if (!is.na(affirmative_map[varname])) affirmative_map[varname] else 2
      pct <- mean(xnum == pos, na.rm = TRUE) * 100
    } else {
      pos <- if (!is.na(affirmative_map[varname])) affirmative_map[varname] else max(u)
      pct <- mean(xnum == pos, na.rm = TRUE) * 100
    }
    list(N = N, pct = round(pct, 1), rng = rng)
  }
  
  desc_binaria <- function(.df, vars, etiquetas){
    stopifnot(length(vars) == length(etiquetas))
    rows <- lapply(seq_along(vars), function(i){
      st <- bin_stats(.df[[ vars[i] ]], vars[i])
      data.frame(
  ...
  desc_ordinal_cont <- function(.df, vars, etiquetas, rango_manual = NULL){
    stopifnot(length(vars) == length(etiquetas))
    N    <- sapply(vars, function(v) sum(!is.na(.df[[v]])))
    Mean <- sapply(vars, function(v) mean(to_num(.df[[v]]), na.rm = TRUE))
    SD   <- sapply(vars, function(v)  sd(to_num(.df[[v]]),   na.rm = TRUE))
    Min  <- sapply(vars, function(v) suppressWarnings(min(to_num(.df[[v]]), na.rm = TRUE)))
    Max  <- sapply(vars, function(v) suppressWarnings(max(to_num(.df[[v]]), na.rm = TRUE)))
    Rng  <- if (!is.null(rango_manual)) rango_manual else paste0(Min, "–", Max)
    data.frame(Variable = etiquetas, N = N,
               Media = round(Mean, 2), `Desvío` = round(SD, 2), Rango = Rng,
               check.names = FALSE)
  }
  
  tema_gt <- function(tab){
    tab |>
      tab_options(
        table.font.names = c("Latin Modern Roman","Times New Roman","Serif"),
        table.width = px(760),
  ...
  nota_base <- "Notas: N calculado sobre observaciones no perdidas. En binarias, el % positivo respeta el esquema 0/1. Escalas ordinales según rango indicado. Contextuales: BM, PNUD, V-Dem."
  
  # ========= Paneles =========
  
  # A) Dependientes (usa 'juntas_alto')
  panelA <- desc_binaria(
    df,
    vars = c("voto","partprot","juntas_alto"),
    etiquetas = c("Voto (última elección)","Protesta","Juntas barriales (alto)")
  )
  
  # B1) Percepción (rango corregido: genperc 1–4; partperc 1–5)
  panelB1 <- desc_ordinal_cont(
    df,
    vars = c("genperc","partperc"),
    etiquetas = c("Percepción de corrupción (general)","Percepción de corrupción (particular)"),
    rango_manual = c("1–4","1–5")
  )
  
  # B2) Experiencia (binarias)
  panelB2 <- desc_binaria(
    df,
    vars = c("sobpol","sobpub"),
    etiquetas = c("Experiencia: soborno policía","Experiencia: soborno empleados públicos")
  )
  
  # C) Controles individuales (rangos manuales corregidos)
  panelC <- desc_ordinal_cont(
    df,
    vars = c("genero","edad","educ","income","govhelp","intpol","news","ecoperc",
             "ecopersperc","confint","respinst","job"),
    etiquetas = c("Género (1=Hombre)","Edad (años)","Educación (categorías)","Ingreso (categorías)",
                  "Ayuda estatal","Interés en política","Consumo de noticias","Evaluación economía país",
                  "Evaluación economía personal","Confianza interpersonal","Respeto institucional",
                  "Situación laboral (1=ocupado)")
  ) |>
    mutate(
      Rango = dplyr::case_when(
        Variable %in% c("Género (1=Hombre)","Situación laboral (1=ocupado)") ~ "0–1",
        Variable == "Edad (años)" ~ paste0(suppressWarnings(min(df$edad, na.rm=TRUE)),
                                           "–", suppressWarnings(max(df$edad, na.rm=TRUE))),
        Variable == "Educación (categorías)" ~ "0–3",
        Variable == "Ingreso (categorías)"   ~ "0–16",  # <<< corregido
        Variable == "Interés en política"    ~ "1–4",
        Variable == "Consumo de noticias"    ~ "1–5",
        Variable == "Evaluación economía país"     ~ "1–3",  # <<< corregido (ecoperc)
        Variable == "Evaluación economía personal" ~ "1–3",  # <<< corregido (ecopersperc)
        Variable == "Respeto institucional"      ~ "1–7",  # <<< corregido (respinst)
        TRUE ~ Rango
      )
    )
  
  # D) Contextuales
  panelD <- desc_ordinal_cont(
    df,
    vars = c("gdp_pc","desempleo","idh","democracia","enforcement"),
    etiquetas = c("PIB per cápita","Desempleo (%)","Índice de desarrollo humano",
                  "Índice de democracia","Enforcement")
  ) |>
    mutate(Rango = ifelse(Variable == "Enforcement", "0–4", Rango))
  
  # ========= Render GT =========
  gtA  <- panelA |> gt() |> tab_header(title = md("**Panel A. Variables dependientes**")) |> tema_gt() |>
    fmt_number(columns = `% de 1`, decimals = 1) |> tab_source_note(nota_base)
  
  gtB1 <- panelB1 |> gt() |> tab_header(title = md("**Panel B1. Percepción de corrupción**")) |> tema_gt() |>
    fmt_number(columns = c(Media, `Desvío`), decimals = 2) |> tab_source_note(nota_base)
  
  gtB2 <- panelB2 |> gt() |> tab_header(title = md("**Panel B2. Experiencia de corrupción (binarias)**")) |> tema_gt() |>
    fmt_number(columns = `% de 1`, decimals = 1) |> tab_source_note(nota_base)
  
  gtC  <- panelC |> gt() |> tab_header(title = md("**Panel C. Controles individuales**")) |> tema_gt() |>
    fmt_number(columns = c(Media, `Desvío`), decimals = 2) |> tab_source_note(nota_base)
  
  gtD  <- panelD |> gt() |> tab_header(title = md("**Panel D. Controles contextuales (país–año)**")) |> tema_gt() |>
    fmt_number(columns = c(Media, `Desvío`), decimals = 2) |> tab_source_note(nota_base)
  
  # Mostrar
  gtA; gtB1; gtB2; gtC; gtD
  
  # Exportar (opcional)
  # gtsave(gtA,  "panelA_dependientes.png");  gtsave(gtA,  "panelA_dependientes.html")
  # gtsave(gtB1, "panelB1_percepcion.png");   gtsave(gtB1, "panelB1_percepcion.html")
  # gtsave(gtB2, "panelB2_experiencia.png");  gtsave(gtB2, "panelB2_experiencia.html")
  # gtsave(gtC,  "panelC_controles.png");     gtsave(gtC,  "panelC_controles.html")
  # gtsave(gtD,  "panelD_contextuales.png");  gtsave(gtD,  "panelD_contextuales.html")
